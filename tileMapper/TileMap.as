		...
		public static function findPath(mainMap:Array, startP:Point, endP:Point, diagonal:Boolean, diagonalWall:Boolean) {
			...
			while (ts != te) {
				for (i=ts+1; i<te; i += 2) {
					...
					if (tiles[i - 1] > 0 && tileTypes[map[tiles[i]][tiles[i - 1] - 1]] == "s") {
						map[tiles[i]][tiles[i - 1] - 1] = {y:tiles[i],x:tiles[i - 1]};
						tiles.push(tiles[i-1]-1, tiles[i]);
					}
					if (tiles[i] < ROWS && tileTypes[map[tiles[i] + 1][tiles[i - 1]]] == "s") {
						map[tiles[i] + 1][tiles[i - 1]] = {y:tiles[i],x:tiles[i - 1]};
						tiles.push(tiles[i-1], tiles[i]+1);
					}
					if (tiles[i] > 0 && tileTypes[map[tiles[i] - 1][tiles[i - 1]]] == "s") {
						map[tiles[i] - 1][tiles[i - 1]] = {y:tiles[i],x:tiles[i - 1]};
						tiles.push(tiles[i-1], tiles[i]-1);
					}
					if (diagonal==true) {
						if (tiles[i] > 0 && tiles[i - 1] > 0 && tileTypes[map[tiles[i] - 1][tiles[i - 1] - 1]] == "s") {
							/*if (diagonalWall) {
								if (tileTypes[map[tiles[i]][tiles[i - 1] - 1]] != 1 && map[tiles[i] - 1][tiles[i - 1]] != 1) {
						...
						if (tiles[i] < ROWS && tiles[i - 1] < COLS && tileTypes[map[tiles[i] + 1][tiles[i - 1] + 1]] == "s") {
							/*if (diagonalWall) {
						...
						if (tiles[i] < ROWS && tiles[i - 1] > 0 && tileTypes[map[tiles[i] + 1][tiles[i - 1] - 1]] == "s") {
							/*if (diagonalWall) {
								if (map[tiles[i]][tiles[i - 1] - 1] != 1 && map[tiles[i] + 1][tiles[i - 1]] != 1) {
									map[tiles[i] + 1][tiles[i - 1] - 1] = {y:tiles[i],x:tiles[i - 1]};
									tiles.push(tiles[i-1]-1, tiles[i]+1);
						...
						if (tiles[i] > 0 && tiles[i - 1] < COLS && tileTypes[map[tiles[i] - 1][tiles[i - 1] + 1]] == "s") {
							/*if (diagonalWall) {
								if (map[tiles[i]][tiles[i - 1] + 1] != 1 && map[tiles[i] - 1][tiles[i - 1]] != 1) {
									map[tiles[i] - 1][tiles[i - 1] + 1] = {y:tiles[i],x:tiles[i - 1]};
									tiles.push(tiles[i-1]+1, tiles[i]-1);
